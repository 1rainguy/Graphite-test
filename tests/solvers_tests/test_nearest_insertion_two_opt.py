...
from solvers import NearestInsertionTwoOptSolver
from solvers.common_utils import geom_edges, get_tour_length
from tests.utils import read_prob


def main():
    solver = NearestInsertionTwoOptSolver(time_limit=5)

    results_path = os.path.join(PROJECT_ROOT, 'results')
    os.makedirs(results_path, exist_ok=True)
    out_csv = os.path.join(results_path, 'nearest_insertion_two_opt_results.csv')
    ...
    for pid in range(10):
        ...
        print(f'NearestInsertion+2opt problem {pid} route length: {total_length}')
