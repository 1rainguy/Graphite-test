import os
import sys
import importlib
import asyncio
import numpy as np
import random

PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))  # adjust as needed
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from solvers import TwoOptSolver
from solvers.common_utils import geom_edges
from solvers.common_utils import get_tour_length
from tests.utils import read_prob
PROBLEM_NUM = 10

def main():
    solver = TwoOptSolver(time_limit=10)

    problem_ids = range(PROBLEM_NUM)
    for problem_id in problem_ids:
        coords = read_prob(f'problems/{problem_id}.prob')
        dist = geom_edges(coords)
        route = solver.solve(dist)
        total_length = get_tour_length(route, dist)
        with open('results/two_opt_results.csv', 'a') as f:
            f.write(f"{problem_id},{total_length}\n")
        print(f'Problem {problem_id} route length: {total_length}')

if __name__ == '__main__':
    main()
