...
from solvers import AdvancedGeneticSolver
from solvers.common_utils import geom_edges, get_tour_length
from tests.utils import read_prob


def main():
    solver = AdvancedGeneticSolver(time_limit=5, population_size=60, generations=120)

    results_path = os.path.join(PROJECT_ROOT, 'results')
    os.makedirs(results_path, exist_ok=True)
    out_csv = os.path.join(results_path, 'advanced_genetic_results.csv')
    ...
    for pid in range(10):
        ...
        print(f'AdvancedGA problem {pid} route length: {total_length}')
