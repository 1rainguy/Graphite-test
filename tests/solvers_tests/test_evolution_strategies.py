...
from solvers import EvolutionStrategiesSolver
from solvers.common_utils import geom_edges, get_tour_length
from tests.utils import read_prob


def main():
    solver = EvolutionStrategiesSolver(time_limit=5, population_size=40, generations=100, mu=15, lambda_=30)

    # Run over 10 problems and write results CSV
    results_path = os.path.join(PROJECT_ROOT, 'results')
    os.makedirs(results_path, exist_ok=True)
    out_csv = os.path.join(results_path, 'evolution_strategies_results.csv')
    ...
    for pid in range(10):
        ...
        print(f'EvolutionStrategies problem {pid} route length: {total_length}')
